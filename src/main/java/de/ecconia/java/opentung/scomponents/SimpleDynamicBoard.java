...
public class SimpleDynamicBoard extends SComponent
{
	...
	private void placeCube(float cx, float cy, float cz, float hw, float hh, float hd, IntHolder o)
	{
		...
		addVertex(cx + hw, cy - hh, cz - hd, 1, 0, 1, +0, +0, -1, o.getAndInc(9)); //1 Right Bottom Front
		addVertex(cx + hw, cy + hh, cz - hd, 1, 1, 1, +0, +0, -1, o.getAndInc(9)); //5 Right Top    Front
		addVertex(cx - hw, cy + hh, cz - hd, 0, 1, 1, +0, +0, -1, o.getAndInc(9)); //6 Left  Top    Front
		//Right:
		addVertex(cx + hw, cy - hh, cz - hd, 0, 0, 1, +1, +0, +0, o.getAndInc(9)); //1 Right Bottom Front
		addVertex(cx + hw, cy - hh, cz + hd, 1, 0, 1, +1, +0, +0, o.getAndInc(9)); //0 Right Bottom Back
		addVertex(cx + hw, cy + hh, cz + hd, 1, 1, 1, +1, +0, +0, o.getAndInc(9)); //4 Right Top    Back
		addVertex(cx + hw, cy + hh, cz - hd, 0, 1, 1, +1, +0, +0, o.getAndInc(9)); //5 Right Top    Front
		//Back:
		addVertex(cx + hw, cy - hh, cz + hd, 0, 0, 1, +0, +0, +1, o.getAndInc(9)); //0 Right Bottom Back
		addVertex(cx - hw, cy - hh, cz + hd, 1, 0, 1, +0, +0, +1, o.getAndInc(9)); //3 Left  Bottom Back
		addVertex(cx - hw, cy + hh, cz + hd, 1, 1, 1, +0, +0, +1, o.getAndInc(9)); //7 Left  Top    Back
		addVertex(cx + hw, cy + hh, cz + hd, 0, 1, 1, +0, +0, +1, o.getAndInc(9)); //4 Right Top    Back
		// Left:
		addVertex(cx - hw, cy - hh, cz + hd, 0, 0, 1, -1, +0, +0, o.getAndInc(9)); //3 Left  Bottom Back
		addVertex(cx - hw, cy - hh, cz - hd, 1, 0, 1, -1, +0, +0, o.getAndInc(9)); //2 Left  Bottom Front
		addVertex(cx - hw, cy + hh, cz - hd, 1, 1, 1, -1, +0, +0, o.getAndInc(9)); //6 Left  Top    Front
		addVertex(cx - hw, cy + hh, cz + hd, 0, 1, 1, -1, +0, +0, o.getAndInc(9)); //7 Left  Top    Back
		//Up:
	...
	private void addVertex(float x, float y, float z, float nx, float ny, float nz, float tx, float ty, float flag, int o)
	{
		vertices[o++] = x; //X
		vertices[o++] = y; //Y
		vertices[o++] = z; //Z
