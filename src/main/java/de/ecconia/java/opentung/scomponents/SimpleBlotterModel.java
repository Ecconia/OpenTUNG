...
public class SimpleBlotterModel extends SComponent
{
	...
	public SimpleBlotterModel()
	{
		...
		placeCube(0, 0, -(0.15f + 0.15f), 0.045f, 0.045f, 0.15f, 0.0f, 0.0f, 0.0f, offset, Direction.zP); //Input
		placeCube(0, 0, 0.15f + 0.06f, 0.075f, 0.075f, 0.06f, 1.0f, 0.0f, 0.0f, offset, Direction.zN); //Output
		
		offset = new IntHolder(); //Indices array offset
		indices = new short[3 * 6 * 6 - 2 * 6];
		placeCubeIndices(offset, 0, null); //Base
		placeCubeIndices(offset, 24, Direction.zP); //Input
		placeCubeIndices(offset, 48 - 4, Direction.zN); //Output
