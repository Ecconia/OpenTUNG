...
void main()
{
    ...
    vec3 sourceAmbient = vec3(0.0, 0.0, 0.0);
    vec3 sourceDiffuse = vec3(1.0, 1.0, 1.0);

    vec3 materialAmbient = vec3(0.0, 0.0, 0.0);
    vec3 materialDiffuse = tColor.rgb;

    //Loop:
    vec3 lightPosition = (view2 * vec4(sourcePosition, 1.0)).xyz;
